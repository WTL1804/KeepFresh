...
- (void)viewDidLoad {
    ...
    _optionsArray = [[NSArray alloc] initWithObjects:@"过期物品",@"删除物品",@"耗尽物品",nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_cell == nil) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(40, 0, _cell.frame.size.width - 40, _cell.frame.size.height)];
        [_cell.contentView addSubview:label];
        label.text = _optionsArray[indexPath.section];
        label.textAlignment = NSTextAlignmentNatural;
        _cell.contentView.alpha = 1;
      NSArray *imageNameArray = [[NSArray alloc] initWithObjects:@"yiguoqi.png",@"shanchu.png",@"xiaohaopin.png",nil];
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageNameArray[indexPath.section]]];
