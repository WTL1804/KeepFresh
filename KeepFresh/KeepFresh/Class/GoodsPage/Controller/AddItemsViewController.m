...
- (void)viewDidLoad {
    ...
    [self addObserver:[KeepFreshManage sharedLeton] forKeyPath:@"access_token" options:NSKeyValueObservingOptionNew context:nil];
    [self addObserver:[KeepFreshManage sharedLeton] forKeyPath:@"image" options:NSKeyValueObservingOptionNew context:nil];
...
- (void)addDone {
   ...
   //添加属性
    ...
    //上传照片:
//    [[KeepFreshManage sharedLeton] uploadImageWithItem:self.addItemsView.addItems success:^(UploadImageModel * _Nonnull uploadModel) {
//        NSLog(@"图片上传成功");
...
//    }];
    ...
    [[KeepFreshManage sharedLeton] itemsStoredWithUserID:[defaults valueForKey:@"ID"] JSESSION:[defaults valueForKey:@"jsession"] Items:self.addItemsView.addItems success:^(NSDictionary * _Nonnull dict) {
        NSLog(@"%@", dict);
    } error:^(NSError * _Nonnull error) {
...
- (void)dealloc {
    [self removeObserver:[KeepFreshManage sharedLeton] forKeyPath:@"access_token"];
     [self removeObserver:[KeepFreshManage sharedLeton] forKeyPath:@"image"];
}
/*
